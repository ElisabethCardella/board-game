...
export class Player {
  ...
  movePlayer(newRow, newColumn) {
    ...
    if (arm) {
      ...
      var playerArmImageElement = document.getElementById(
        this.name + "ArmImage"
      );
  ...
  tryToAttack() {
    const isOtherPlayerBelow =
      this.row + 1 === this.otherPlayer.row &&
      this.column === this.otherPlayer.column;
    const isOtherPlayerAbove =
      this.row - 1 === this.otherPlayer.row &&
      this.column === this.otherPlayer.column;
    const isOtherPlayertoTheRight =
      this.row === this.otherPlayer.row &&
      this.column + 1 === this.otherPlayer.column;
    const isOtherPlayerToTheLeft =
      this.row === this.otherPlayer.row &&
      this.column - 1 === this.otherPlayer.column;
    ...
    if (
      isOtherPlayerBelow ||
      isOtherPlayerAbove ||
      isOtherPlayertoTheRight ||
      isOtherPlayerToTheLeft
    ) {
      $("#myModal").modal("show");
  ...
  beginTurn() {
    ...
    for (let offset = 1; offset <= 3; offset++) {
      ...
      if (
        !cellElement.hasClass("inaccessible") &&
        !cellElement.hasClass("player")
      ) {
